...
import { Search, Grid3x3, List, ShoppingCart, Star, SlidersHorizontal } from "lucide-react"
...
export default function MedicinesPage() {
  ...
  const categories = ["All", "Pain Relief", "Antibiotics", "Vitamins", "Allergy", "Digestive", "Diabetes"]
            ...
            <div>
              ...
              {viewMode === "grid" ? (
                  ...
                  {medicines.map((medicine) => (
                    <Card key={medicine.id} className="group overflow-hidden transition-shadow hover:shadow-lg">
                      ...
                      <CardFooter className="p-4 pt-0">
                        <Button className="w-full" size="sm" disabled={!medicine.stock}>
                ...
                <div className="space-y-4">
                  {medicines.map((medicine) => (
                    <Card key={medicine.id} className="overflow-hidden transition-shadow hover:shadow-lg">
                      <CardContent className="p-4">
                        <div className="flex gap-4">
                          <Link href={`/medicines/${medicine.id}`} className="shrink-0">
                          ...
                          <div className="flex flex-1 flex-col justify-between">
                            <div>
                              <div className="mb-1 flex items-start gap-2">
                                <Link href={`/medicines/${medicine.id}`} className="flex-1">
                                  <h3 className="font-semibold hover:text-primary">{medicine.name}</h3>
                                </Link>
                                {medicine.prescription && (
                                ...
                                {medicine.discount > 0 && (
                                  <Badge className="shrink-0 bg-primary">{medicine.discount}% OFF</Badge>
                                )}
                              </div>
                              <p className="text-sm text-muted-foreground">{medicine.brand}</p>
                            ...
                            <div className="mt-2 flex items-center justify-between">
                              ...
                              <Button size="sm" disabled={!medicine.stock}>
                                <ShoppingCart className="mr-2 h-4 w-4" />
                                {medicine.stock ? "Add to Cart" : "Out of Stock"}
                              </Button>
                            </div>
